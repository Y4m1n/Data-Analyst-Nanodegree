...
```{r}
#character to date
head(as.Date(dt$contb_receipt_dt,format='%d-%b-%y'))
```
## clean the data
```{r}
...
```{r}
dt.oc<-dt.c%>%
  subset(!is.na(dt.c$contbr_occupation) & length(dt.c$contbr_occupation)>1)%>%
...
#plot
ggplot(subset(dt.oc,n>1000),aes(x=contbr_occupation, y=n,fill=contbr_occupation))+
